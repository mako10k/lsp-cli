...
test("cli format-range can dry-run and apply formatting edits", { timeout: 10_000 }, async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "lsp-cli-format-range-"));
  ...
  // Dry-run
  {
    const res = await runCli(
      ["--root", root, "--server", "mock", "--config", cfgPath, "--format", "json", "format-range", file, "0", "0", "0", "0"],
      { timeoutMs: 5000 }
    );
