...
test("cli implementation returns locations", { timeout: 10_000 }, async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "lsp-cli-impl-"));
  ...
  const res = await runCli(["--root", root, "--server", "mock", "--config", cfgPath, "--format", "json", "implementation", file, "0", "0"], {
    timeoutMs: 5000
  });
